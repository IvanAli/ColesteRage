...
public abstract class Entity {
    ...
    private void checkRailChange() {
        ...
        if(changingUpperRail) {
            ...
            if(ydest < positionsy[nextRail]) {
                currentRail--;
                y = positionsy[currentRail];
                dy = 0;
                moving = changingUpperRail = false;
            }
        }
        if(changingLowerRail) {
        	dy += moveSpeed;
            if(dy > maxSpeed) dy = maxSpeed;
            if(ydest > positionsy[nextRail]) {
                currentRail++;
                y = positionsy[currentRail];
                dy = 0;
                moving = changingLowerRail = false;
            }
        }
        if(moving) return;
        if(up) {
            if(currentRail != UPPER_RAIL) {
                ...
                System.out.println("Next rail: " + nextRail);
            }
            up = false;
        }
        else if(down) {
    ...
    public int getNextRail() { return nextRail; }
    public int getWidth() { return width; }
    public int getHeight() { return height; }
    public int getCWidth() { return cwidth; }
    public int getCHeight() { return cheight; }
    public long getAnimationDelay() { return animationDelay; }
    public boolean isChangingUpperRail() { return changingUpperRail; }
    public boolean isChangingLowerRail() { return changingLowerRail; }
