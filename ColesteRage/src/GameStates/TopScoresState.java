package GameStates;
import java.awt.Graphics2D;
import java.awt.Color;

import Audio.AudioLoader;
import Audio.AudioPlayer;
import Background.Background;
import Game.GamePanel;
import HUD.HUD;
import Images.ImagesLoader;
import Keyboard.Keys;
import StateMisc.IOPlayers;
import Text.Text;

public class TopScoresState extends GameState {
	
	private Background bg;
	private Color[] colors;
	
	private int ticks;
	private int colorTick;
	
	
	public TopScoresState(GameStateManager gsm) {
		
		super(gsm);
		init();
		
	}
	
	public void init() {
		
		// init the images loader
		ImagesLoader.loadFromFile("topscores_imagenes.txt");
		
		bg = new Background(
				0,
				0,
				ImagesLoader.getImage("bgtopscores"),
				Background.MOVEUP
		);
		bg.setVector(0, 2);
		
		// init the audio
		AudioLoader.loadFromFile("topscores_sonido.txt");
		
		// play the audio
		AudioPlayer.playAndLoop("topscoresmusic");
		
		// colors init
		colors = new Color[5];
		colors[0] = new Color(235, 53, 53, 200);
		colors[1] = new Color(255, 207, 93, 200);
		colors[2] = new Color(212, 255, 93, 200);
		colors[3] = new Color(64, 183, 234, 200);
		colors[4] = new Color(255, 93, 120, 200);
		
		IOPlayers.readTopPlayers();
		
	}
	
	public void update() {
		input();
		bg.update();
		ticks++;
		ticks %= 18;
		if(ticks == 0) {
			colorTick++;
		}
	}
	
	public void draw(Graphics2D g) {
		
		// draw the background
		bg.draw(g);
		
		// draw the names background
		/*g.drawImage(
				ImagesLoader.getImage("namesbg"),
				GamePanel.WIDTH / 2 - ImagesLoader.getImage("namesbg").getWidth() / 2,
				380 - ImagesLoader.getImage("namesbg").getHeight() / 2,
				null
		);*/
		g.setColor(new Color(255, 255, 255, 160));
		g.fillRect(0, 185, GamePanel.WIDTH, 410);
		for(int i = 0; i < colors.length; i++) {
			int pos = (i + colorTick) % colors.length;
			g.setColor(colors[pos]);
			g.fillRect(0, 195 + i * 80, GamePanel.WIDTH, 70);
		}
		
		Text.drawString(g, "Mejores puntajes", Text.CENTERED, 100, "leishoFont", null);
		
		for(int i = 0; i < IOPlayers.playersSize(); i++) {
			String name = IOPlayers.getPlayerAt(i).getName();
			String score = HUD.getScore(IOPlayers.getPlayerAt(i).getScore());
			Text.drawString(
					g,
					(i + 1) + ". " + name, 
					200, 
					200 + i * 80,
					"leishoFont", 
					null
			);
			Text.drawString(
					g,
					score, 
					850, 
					200 + i * 80,
					"leishoFont", 
					null
			);
		}
		if(ticks < 9)
			Text.drawString(g, "Presiona Enter para regresar", Text.CENTERED, 640, "leishoFont", null);
	}
	
	public void input() {
		if(Keys.isPressed(Keys.ENTER)) {
			AudioPlayer.play("menu_back");
			AudioPlayer.stop("topscoresmusic");
			ImagesLoader.removeImages("topscores_imagenes.txt");
			AudioLoader.removeAudio("topscores_sonido.txt");
			gsm.setState(GameStateManager.MENU);
		}
	}
	
	public void clearAll() {
    	ImagesLoader.clearImages();
    	AudioPlayer.clearAudio();
    }
	
}
