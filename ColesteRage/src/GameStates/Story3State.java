...
public class Story3State extends GameState {
	...
	private final String musicFnm = "story3music";
	...
	public Story3State(GameStateManager gsm) {
	...
	public void init() {
		...
		ImagesLoader.loadFromFile("historia3_imagenes.txt");
		AudioLoader.loadFromFile("historia3_sonido.txt");
		
		AudioPlayer.playAndLoop(musicFnm);
		
		dialogs = new String[NUM_DIALOGS];
		
		dialogs[0] = "Arrasó con los alimentos de su escuela y no quedó satisfecho con ello.";
		dialogs[1] = "Entonces, Bodoque salió corriendo hacia un supermercado, donde había comida chatarra casi ilimitada.";
	...
	public void update() {
		...
		if(ticks == FADEIN + FADEOUT) {
			...
			ImagesLoader.removeImages("historia3_imagenes.txt");
			AudioLoader.removeAudio("historia3_sonido.txt");
