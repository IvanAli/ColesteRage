...
public class NameEntryState extends GameState {
	...
	public void init() {
		imsLoader = new ImagesLoader("nameentry_imagenes.txt");
		...
		foodPattern = new Background(
				0, 
				0, 
				imsLoader.getImage("foodpattern"), 
				Background.MOVEUP
		);
	...
	public void update() {
		...
		if(transitioning) {
			...
			if(ticksFadeOut > 40) gsm.setState(GameStateManager.MENU);
		}
	...
	public void draw(Graphics2D g) {
		...
		if(saved || !topFive) {
			...
			// draw the background
			//g.drawImage(imsLoader.getImage("foodpattern"), 0, 0, null);
			...
			*/
			if(currentOptiony < 4)
			g.drawImage(
					imsLoader.getImage("letterBox"),
					100 + currentOptionx * 90 - imsLoader.getImage("letterBox").getWidth() / 2,
					300 + currentOptiony * 90 - imsLoader.getImage("letterBox").getHeight() / 2,
					null
			);
			
			// draw the textbox
			g.drawImage(
					imsLoader.getImage("textbox"),
					GamePanel.WIDTH / 2 + 30 - imsLoader.getImage("textbox").getWidth() / 2,
					225 - imsLoader.getImage("textbox").getHeight() / 2,
					null
			);
