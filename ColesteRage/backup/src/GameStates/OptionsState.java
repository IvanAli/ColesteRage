...
public class OptionsState extends GameState {
	...
	public void init() {
		...
		imsLoader = new ImagesLoader("options_imagenes.txt");
		bg = new Background(
				0,
				0,
				imsLoader.getImage("bgoptions"),
				Background.MOVEUP
		);
		bg.setVector(0, 2);
		
		// init the audio
		audio = new AudioPlayer(new AudioLoader("options_sonido.txt"));
		audio.playAndLoop("optionsmusic");
		...
		if(enter) {
			if(currentOption == SAVE) {
				...
				if(soundOption == ON) {
					...
					if(!audio.isPlaying("optionsmusic")) {
