...
public class Player extends Entity {
    ...
    public Player(ImagesLoader imsLoader, AudioPlayer audio) {
        super(imsLoader);
    ...
    public void init(ArrayList<Dish> dishes, BackgroundManager bgManager, ArrayList<DishValue> numbers, ArrayList<SpriteAnimation> explosions) {
        ...
        // WALKING
        sprites.add(imsLoader.getFrames("player_walk"));
        delays.add(150);
        // EATING HEALTHY
        sprites.add(imsLoader.getFrames("player_eatinghealthy"));
        delays.add(150);
        // EATING UNHEALTHY
        sprites.add(imsLoader.getFrames("player_eatingunhealthy"));
        delays.add(150);
        // EATING POWER
        sprites.add(imsLoader.getFrames("player_eatingpower"));
        delays.add(150);
        // JUMPING
        sprites.add(imsLoader.getFrames("player_jump"));
        delays.add(-1);
        // FALLING
        sprites.add(imsLoader.getFrames("player_fall"));
        delays.add(-1);
        // CHANGING RAIL
        sprites.add(imsLoader.getFrames("player_changerail"));
        delays.add(-1);
    ...
    public void update() {
        ...
        if(caught) {
            ...
            if(currentAction != EATINGPOWER) {
                ...
                setAnimation("player_eatingpower", 150);
            ...
            if(currentAction != EATINGHEALTHY) {
                ...
                setAnimation("player_eatinghealthy", 150);
            ...
            if(currentAction != EATINGUNHEALTHY) {
                ...
                setAnimation("player_eatingunhealthy", 150);
        ...
        */
        ...
        if(playing) {
            ...
            if(timeElapsed >= 1000) {
                time--;
                if(time < 6 && time > 0) audio.play("timecounter");
                if(time == 0) audio.play("timeend");
                if(time < 0) {
                    time = 0;
                ...
                if(eats(d)) {
                    if(d instanceof Food) {
                        ...
                        if(f.getValue() > 0) {
                            ...
                            explosions.add(new SpriteAnimation(f.getx(), f.gety(), imsLoader, "explosionUnhealthy", 50, camera));
                            audio.play("eatingunhealthy");
                        }
                        if(f.getValue() < 0) {
                            eatinghealthy = true;
                            explosions.add(new SpriteAnimation(f.getx(), f.gety(), imsLoader, "explosionHealthy", 50, camera));
                            audio.play("eatinghealthy");
                        }
                        eatingTimer = System.nanoTime();
                        numbers.add(new DishValue(f.getx(), f.gety(), f.getValue(), imsLoader, camera));
                    ...
                    if(d instanceof Power) {
                        ...
                        numbers.add(new DishValue(d.getx(), d.gety(), 0, imsLoader, camera));
                        explosions.add(new SpriteAnimation(d.getx(), d.gety(), imsLoader, "explosionPower2", 50, camera));
                    }
                    dishes.remove(i);
                    i--;
                }
